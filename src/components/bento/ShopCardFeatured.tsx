...
export function ShopCardFeatured({
  ...
  const categoryTagline =
    shop.categories && shop.categories.length > 0
      ...
      : 'Top-rated artisan café featuring signature egg coffee, slow pour-overs, and a tranquil courtyard workspace.';
        ...
        <div className="flex items-center justify-end gap-2 pt-0.5">
          ...
          <Link href={APP_ROUTES.SHOP_DETAIL(shop.id)} onClick={(e) => e.stopPropagation()}>
            <Button
              variant="default"
              size="sm"
              className="h-8 px-3.5 text-xs bg-amber-gold text-dark-bg font-bold hover:bg-amber-gold-hover rounded-xl shadow-md transition-all"
            >
