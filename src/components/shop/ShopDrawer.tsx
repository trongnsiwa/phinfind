...
import {
    ...
    Phone, Quote, Send, Share2, Sparkles, Star, Sun, Utensils, Wifi, Wind, X, Zap
} from 'lucide-react';
  ...
  return (
    <div className='space-y-4 pb-16'>
      ...
      <div className='bg-gradient-to-r from-amber-gold/15 via-dark-roast to-dark-roast/80 p-3.5 rounded-2xl border border-amber-gold/30 space-y-2 shadow-inner'>
        ...
        <p className='text-xs text-soft-beige leading-relaxed font-medium'>{experienceTagline}</p>
      </div>
      ...
      <div className='space-y-2'>
        <div className='flex items-center justify-between'>
          ...
          <span className='text-[11px] text-amber-gold font-medium'>4.8 ★★★★★</span>
        </div>
...
// Memoized Reviews Tab with Interactive Review Submission & Guest Gate
const ReviewsTab = memo(function ReviewsTab({ shop }: { shop: CoffeeShop }) {
  ...
  // Fetch verified shop reviews from Supabase API on mount
  useEffect(() => {
    let isMounted = true;
    const fetchReviews = async () => {
      try {
        ...
        // Fallback to MOCK_REVIEWS gracefully
      }
    };
    fetchReviews();
    return () => {
    ...
    <div className='space-y-4 pb-16'>
      ...
      <div className='bg-dark-roast/50 p-4 rounded-2xl border border-dark-border/60 flex flex-col sm:flex-row items-center gap-4 shadow-sm'>
        <div className='text-center sm:text-left flex flex-col items-center sm:items-start min-w-[100px]'>
          ...
          <span className='text-[11px] text-warm-gray'>{totalReviews} Verified Reviews</span>
        </div>
        ...
        <div className='flex-1 w-full space-y-1.5 text-xs text-soft-beige'>
          {[
            ...
            <div key={bar.star} className='flex items-center gap-2'>
              ...
              <span className='w-8 text-right text-[10px] text-warm-gray font-medium'>
                {bar.pct}%
              </span>
            </div>
          ))}
      ...
      {/* Write a Review Action / Guest Auth Prompt */}
      {!isAuthenticated ? (
        <div className='bg-dark-roast/40 p-3.5 rounded-2xl border border-amber-gold/30 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-sm'>
          <div className='flex items-center gap-3'>
            <div className='w-9 h-9 rounded-xl bg-amber-gold/15 border border-amber-gold/30 flex items-center justify-center text-amber-gold flex-shrink-0'>
              <Edit3 size={17} />
            </div>
            <div>
              ...
              <span className='text-[11px] text-soft-beige/75'>
                Sign in to rate brews, share atmosphere notes & post your review.
              </span>
            </div>
          </div>
          <Link
          ...
          >
            <Button
              type='button'
              variant='outline'
              className='w-full sm:w-auto bg-amber-gold/20 hover:bg-amber-gold/30 text-amber-gold hover:text-amber-gold-hover border-amber-gold/40 hover:border-amber-gold/60 text-xs font-bold rounded-xl px-4 py-2 flex items-center justify-center gap-1.5 transition-all active:scale-95 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-gold focus-visible:ring-offset-2 focus-visible:ring-offset-dark-bg'
            >
              <LogIn size={14} />
        ...
        <form
          ...
          className='bg-dark-roast/70 p-4 rounded-2xl border border-amber-gold/40 shadow-lg space-y-3.5 animate-in fade-in zoom-in-95 duration-200'
        >
          ...
          <div className='space-y-1'>
            ...
            <textarea
              ...
              rows={3}
              className='w-full bg-dark-bg/80 border border-dark-border/80 rounded-xl p-3 text-xs text-cream-white placeholder:text-warm-gray/60 focus:outline-none focus:border-amber-gold resize-none transition-colors'
            />
          </div>
          ...
          <div className='flex items-center justify-end gap-2 pt-1'>
            ...
            <Button
              ...
              className='bg-amber-gold hover:bg-amber-gold-hover text-dark-bg font-bold text-xs rounded-xl px-4 py-1.5 h-8 shadow-md flex items-center gap-1.5 disabled:opacity-50 cursor-pointer'
            >
              {isSubmitting ? (
                <>
        ...
        <div className='flex items-center justify-between bg-dark-roast/40 p-3.5 rounded-2xl border border-dark-border/60'>
          <div className='flex items-center gap-2.5'>
          ...
          <Button
            ...
            className='bg-amber-gold hover:bg-amber-gold-hover text-dark-bg font-bold text-xs rounded-xl px-3.5 py-1.5 h-9 shadow-md flex items-center gap-1.5 active:scale-95 transition-all cursor-pointer'
          >
            <Edit3 size={14} />
            Write a review
          </Button>
      ...
      <div className='space-y-3'>
        ...
        {reviewsList.map((rev, idx) => (
          <div
            key={rev.id || idx}
            className={cn(
              'p-3.5 rounded-2xl border space-y-2 text-xs transition-all',
              rev.isUserSubmission
                ? 'bg-amber-gold/10 border-amber-gold/40 shadow-md'
                : 'bg-dark-roast/40 border-dark-border/50'
            )}
          >
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-2'>
                ...
                {rev.isUserSubmission && (
                  <span className='text-[9px] bg-amber-gold text-dark-bg font-bold px-1.5 py-0.2 rounded uppercase tracking-wider'>
                    You
                  </span>
                )}
              </div>
...
export function ShopDrawer({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'photos' | 'reviews' | 'amenities'>(
    'overview'
  );
            ...
            <div className='flex-shrink-0 px-4 sm:px-6 pt-0 pb-2 space-y-2 bg-dark-bg/95 backdrop-blur-xl z-10 border-b border-dark-border/40 select-none'>
              ...
              <div className='space-y-1'>
                ...
                <DrawerPrimitive.Description
                  id='shop-drawer-description'
                  className='text-xs text-soft-beige/85 flex items-start gap-1'
                >
        ...
        >
          <div className='grid grid-cols-3 gap-2.5'>
            <a
              ...
              target='_blank'
              rel='noopener noreferrer'
              className='flex items-center justify-center gap-2 py-2.5 px-3 rounded-full bg-amber-gold text-dark-bg font-bold hover:bg-amber-gold-hover transition-all text-xs shadow-md group active:scale-95 min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-gold focus-visible:ring-offset-2 focus-visible:ring-offset-dark-bg'
            >
              <Navigation
                size={15}
            ...
            <button
              ...
              className={cn(
                'flex items-center justify-center gap-2 py-2.5 px-3 rounded-full border transition-all text-xs font-semibold shadow-xs active:scale-95 min-h-[44px] cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-gold focus-visible:ring-offset-2 focus-visible:ring-offset-dark-bg',
                isFavorite
