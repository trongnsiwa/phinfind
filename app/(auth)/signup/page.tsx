...
function SignupForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const redirectTo = `${origin}${redirect && redirect !== '/' ? `?redirect=${encodeURIComponent(redirect)}` : ''}`;
      ...
      <CardContent className="p-0 space-y-5">
        <form className="space-y-4" onSubmit={handleSignup}>
          <div className="space-y-2">
            <Label htmlFor="fullname" className="text-soft-beige/80 font-semibold text-xs uppercase tracking-wider">
              Full Name
            </Label>
            <Input
              id="fullname"
        ...
        >
          {isGoogleLoading ? (
            <span className="flex items-center gap-2">
              <span className="w-4 h-4 rounded-full border-2 border-soft-beige/30 border-t-soft-beige animate-spin" />
              Connecting...
            </span>
          ) : (
            <>
