...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: followRows, error } = await publicSupabase
      .from('follows')
      .select('follower_id, created_at, profiles!follows_follower_id_fkey(id, username, full_name, avatar_url, bio)')
      .eq('followee_id', targetUserId)
    ...
    const profiles = (followRows || []).map((row: any) => {
      ...
      return {
        id: prof?.id || row.follower_id,
        username: prof?.username || null,
        full_name: prof?.full_name || null,
        avatar_url: prof?.avatar_url || null,
        bio: prof?.bio || null,
        followed_at: row.created_at,
        is_following: currentUserId ? myFollowedSet.has(prof?.id || row.follower_id) : false,
      };
    });
    ...
    return NextResponse.json(
      { error: error?.message || 'Không thể tải danh sách người theo dõi' },
      { status: 500 }
    );
  }
}
